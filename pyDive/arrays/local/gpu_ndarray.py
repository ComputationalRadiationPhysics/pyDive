...
def copy_non_contiguous(dst, src):
    """Copy ``src`` array to ``dst`` array. Can be a cpu- or a gpu-array.
    A gpu-array may have a non contiguous block of memory, i.e. it may have substancial pitches/strides.
    However a cpu-array must have a contiguous block of memory.
    All four combinations of directions are allowed.
    """
    ...
    if len(src.shape) == 1:
        ...
        if (src.strides[0] != src.shape[1] * src.strides[1] if src_on_gpu else False) or \
           (dst.strides[0] != dst.shape[1] * dst.strides[1] if dst_on_gpu else False):
            # arrays have to be copied plane by plane, because there a substantial pitches/strides
...
class gpu_ndarray(pycuda.gpuarray.GPUArray):
    ...
    def __init__(self, shape, dtype, allocator=cuda.mem_alloc,
        ...
        if type(shape) not in (list, tuple):
            ...
            shape = tuple(shape)
        super(gpu_ndarray, self).__init__(
            shape, np.dtype(dtype), allocator,
            base, gpudata, strides, order)
    ...
    def __elementwise_op__(self, op, *args):
        ...
        args = [arg.copy() if isinstance(arg, pycuda.gpuarray.GPUArray) and not arg.flags.forc else arg
                for arg in args]
...
special_ops_avail = set(name for name in pycuda.gpuarray.GPUArray.__dict__.keys()
                        if name.endswith("__"))

make_special_op = lambda op: lambda self, *args: self.__elementwise_op__(op, *args)

special_ops_dict = {
    op: make_special_op(op) for op in
